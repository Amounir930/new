...
export default function CategoryPage() {
  ...
  const category = useMemo(() => categories.find(c => c.slug === params?.slug), [params?.slug]);
  ...
  const getCategoryProducts = (slug: string): Product[] => {
      if (slug === 'men' || slug === 'women' || slug === 'kids') {
          ...
          return products.filter(p => topLevelCategoryMap[slug]?.includes(p.category));
      }
      return products.filter(p => p.category === slug);
  };

  const categoryProducts = useMemo(() => category ? getCategoryProducts(category.slug) : [], [category]);

  const allBrands = useMemo(() => [...new Set(categoryProducts.map(p => p.brand))], [categoryProducts]);
  const maxPrice = useMemo(() => Math.ceil(Math.max(...categoryProducts.map(p => p.price), 0)), [categoryProducts]);
  ...
  }, [sortOption, selectedBrands, priceRange, selectedRating, categoryProducts]);
  ...
  const hasActiveFilters = selectedBrands.length > 0 || priceRange[0] !== 0 || priceRange[1] !== maxPrice || selectedRating > 0;
  ...
  return (
    <div className="container py-8 md:py-12">
      <header className="mb-8 text-center">
        <h1 className="text-4xl font-bold tracking-tight capitalize">{category.name} {t[language].collection}</h1>
        ...
        <main className="flex-1">
          <div className="mb-6 flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              {t[language].showing} {filteredAndSortedProducts.length} {t[language].of} {categoryProducts.length} {t[language].products}
            </p>
