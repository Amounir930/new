...
export default function LoyaltyPage() {
  ...
  const progress =
    (loyaltyData.points / (loyaltyData.points + loyaltyData.pointsToNextTier)) *
    100;
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-semibold mb-2">
              {t[language].youAreA}{' '}
              <span className="text-primary">{loyaltyData.tier}</span>{' '}
              {t[language].member}
            </p>
            <Progress value={progress} className="w-full" />
            <p className="text-sm text-muted-foreground mt-2">
              {loyaltyData.pointsToNextTier} {t[language].pointsToReach}
            </p>
          </CardContent>
        </Card>
      </div>
            ...
            <TableBody>
              {loyaltyData.history.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    {item.activity.includes('Purchase')
                      ? `${t[language].purchase} ${item.activity.split(' ')[1]}`
                      : t[language].wroteReview}
                  </TableCell>
                  <TableCell
