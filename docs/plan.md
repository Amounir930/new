...
| **Arch-S2** | Tenant Isolation Middleware | NestJS middleware: Extracts subdomain → Validates → SET LOCAL app.current_tenant = {id}. Shared-Schema RLS enforced. | Drizzle ORM 🌧️, RLS 🛡️ | Request to `alpha.apex.localhost` accesses ONLY data matching `tenant_id` in `storefront` schema | 2 |  
...
- **الوظيفة:** تحويل النطاقات المتداخلة (مثل `console.storage...`) إلى نطاقات مسطحة (`storage-admin...`) لتتوافق مع قيود Cloudflare Universal SSL.
- **التأثير:** ضمان حماية SSL بنسبة 100% لكافة الأدوات الإدارية دون مشاكل "Cipher Mismatch".

### 6. **Decentralized Shield Deployment (Island Strategy)**
- **الوظيفة:** توزيع الميدل وير (Middlewares) لتكون معرفة محلياً داخل كل حاوية (Container) بدلاً من مركزية Traefik فقط.
- **التأثير:** وضوح كامل للروابط وتجنب أخطاء 404 الناتجة عن تضارب الرؤية بين الحاويات.

### 7. **SaaS Email & DB Tooling (Mailpit & Adminer Integration)**
- **الوظيفة:** توفير بيئة متكاملة لاختبار رسائل البريد (Mailpit) وفحص قواعد البيانات (Adminer) بشكل سحابي مؤمن عبر IP Whitelist.
