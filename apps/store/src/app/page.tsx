  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white selection:bg-blue-500/30 font-sans overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-[#0A0A0A]/80 backdrop-blur-md border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <a href="https://admin.60sec.shop" className="text-sm font-medium text-gray-300 hover:text-white transition">Admin Login</a>
            <button
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 mb-8">
              ...
              <span className="text-sm font-medium text-gray-300">V2 Platform Now Live</span>
            </div>
            <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight mb-8 leading-tight">
              Launch your e-commerce <br className="hidden md:block" />
              empire in <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">60 seconds</span>.
            </h1>
      ...
      <section className="py-24 px-6 border-t border-white/5 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <PricingCard title="Enterprise" price="Custom" features={['Dedicated Infrastructure', 'SLA Guarantee', 'Custom Blueprints', 'Account Manager']} />
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-6 bg-[#0A0A0A] border-t border-white/5">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-400">Our enterprise support team is ready to help you scale.</p>
          </div>
      ...
      <footer className="border-t border-white/5 py-12 px-6 text-center text-gray-500">
        <p>&copy; {new Date().getFullYear()} 60sec.shop by Apex. All rights reserved.</p>
...
function PricingCard({ title, price, features, featured = false }: { title: string; price: string; features: string[], featured?: boolean }) {
  return (
    <div className={`p-8 rounded-3xl border ${featured ? 'bg-gradient-to-b from-blue-900/20 to-transparent border-blue-500/50 relative' : 'bg-white/5 border-white/10'}`}>
      {featured && <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-blue-500 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">Most Popular</div>}
      ...
      <button className={`w-full py-3 rounded-xl font-semibold transition ${featured ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-white/10 hover:bg-white/20 text-white'}`}>
        Get Started
      </button>
    </div>
  );
