...
export function ProvisioningModal({
  ...
  const onProvisioningSubmit = async (data: ProvisioningFormValues) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const fieldError = result.validationErrors?.find(
          (e: { path?: string }) => e.path?.includes('password')
        );
        const humanMessage =
          fieldError?.message ||
          result.message ||
  ...
  const onVerifySubmit = async (data: OTPFormValues) => {
    ...
    try {
      ...
      if (!res.ok) {
        const fieldError = result.validationErrors?.find(
          (e: { path?: string }) => e.path?.includes('otp')
        );
        const humanMessage =
          fieldError?.message || result.message || 'Failed to verify code';
        throw new Error(humanMessage);
      }
          ...
          {step === 1 ? (
            <div>
              ...
              <p className="text-gray-400 mb-6 text-sm">
                Join thousands of merchants powering their businesses with
                60sec.shop.
              </p>
              ...
              <form
                onSubmit={handleProvSubmit(onProvisioningSubmit)}
                className="space-y-4"
              >
                <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="My Store"
                  />
                  {provErrors.storeName && (
                    <p className="text-red-400 text-xs mt-1">
                      {provErrors.storeName.message}
                    </p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-gray-950 border border-gray-800 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-blue-500 outline-none"
                    />
                    {provErrors.confirmPassword && (
                      <p className="text-red-400 text-xs mt-1">
                        {provErrors.confirmPassword.message}
                      </p>
                    )}
                  </div>
                </div>
                ...
                {provErrors.turnstileToken && (
                  <p className="text-red-400 text-xs text-center">
                    {provErrors.turnstileToken.message}
                  </p>
                )}
            ...
            <div>
              <div className="text-center mb-6">
                ...
                <p className="text-gray-400 text-sm">
                  ...
                  <strong>{watch('email')}</strong>. The code expires in 5
                  minutes.
                </p>
              </div>
