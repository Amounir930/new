...
export default function OrdersPage() {
    ...
    useEffect(() => {
        setOrders([
            { id: 'ORD001', customer: 'Ahmed Zaki', total: '$145.00', status: 'shipped', date: '2026-02-22' },
            { id: 'ORD002', customer: 'Sara Mohamed', total: '$230.50', status: 'processing', date: '2026-02-21' },
            { id: 'ORD003', customer: 'Guest User', total: '$89.99', status: 'pending', date: '2026-02-21' },
        ]);
    }, []);
        ...
        <div className="p-8 space-y-6">
            <header className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-500">Manage customer transactions and shipping.</p>
                </div>
                        ...
                        <TableBody>
                            {orders.map((order) => (
                                <TableRow key={order.id} className="hover:bg-slate-50 transition-colors">
                                    <TableCell className="font-bold text-slate-900 py-6 px-6">{order.id}</TableCell>
                                    ...
                                    <TableCell>
                                        <div className={`inline-flex px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest ${order.status === 'shipped' ? 'bg-green-100 text-green-700' :
