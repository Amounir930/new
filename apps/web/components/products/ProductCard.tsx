      ...
      <div className="group relative">
        <div className="relative aspect-[3/4] w-full overflow-hidden rounded-lg bg-muted">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2">
              {product.tags?.includes('new-arrival') && <Badge>{t[language].new}</Badge>}
        ...
        <div className="mt-4 flex justify-between">
         ...
         <div className="mt-1 flex items-center">
              ...
              <span className="ml-1 text-xs text-muted-foreground">{product.rating} ({product.reviewCount} {t[language].reviews})</span>
          </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col gap-4">
                  ...
                  {Object.entries(product.variants).map(([key, values]) => (
                    <div key={key} className="space-y-2">
                      ...
                      >
                        {values.map(value => (
                          <div key={value}>
                            <RadioGroupItem value={value} id={`qv-${key}-${value}`} className="peer sr-only" />
                            <Label
                ...
                <Button asChild variant="link" className="justify-center">
                    <Link href={`/shop/product/${product.slug}`}>{t[language].fullDetails}</Link>
                </Button>
            </div>
        </div>
      </DialogContent>
