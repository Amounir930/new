...
export default function VisualSearchPage() {
    ...
    const t = {
        en: {
            title: "Visual Search",
            subtitle: "Find similar products by uploading a photo or using your camera.",
            upload: "Upload Image",
        ...
        ar: {
            ...
            cameraRequiredDesc: "يرجى السماح بالوصول إلى الكاميرا لاستخدام هذه الميزة.",
            cancel: "إلغاء",
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            if (searchResult.recommendations && searchResult.recommendations.length > 0) {
                const foundProducts = products.filter(p => searchResult.recommendations.includes(p.name));
        ...
        <div className="container py-12 md:py-16">
            ...
            <div className="mx-auto max-w-4xl">
                            ...
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <div className="flex gap-2">
                                    <Button onClick={handleTakePicture} disabled={!hasCameraPermission}>
                                        <Camera className="mr-2 h-4 w-4" /> {t[language].takePicture}
                                    </Button>
                                     <Button variant="outline" onClick={() => setShowCamera(false)}>{t[language].cancel}</Button>
                                </div>
                            </div>
                        )}
                        ...
                        {imageSrc && !showCamera && (
                             <div className="flex flex-col items-center gap-4">
                                <div className="relative w-full max-w-md border rounded-lg p-2">
                                     <Image src={imageSrc} alt={t[language].imagePreviewAlt} width={500} height={500} className="rounded-md object-contain max-h-96 w-auto mx-auto"/>
                                     <Button variant="ghost" size="icon" className="absolute top-2 right-2 rounded-full" onClick={clearImage}>
                                         <X className="h-5 w-5"/>
                ...
                {results.length > 0 && !isLoading && (
                    <div className="mt-12">
                        <h2 className="text-2xl font-bold tracking-tight text-center">{t[language].resultsTitle}</h2>
                        {reasoning && <p className="text-muted-foreground text-center mt-1">{reasoning}</p>}
                ...
                {results.length === 0 && reasoning && !isLoading && !error && (
                     <div className="mt-12 text-center">
                        ...
                        <p className="text-muted-foreground mt-2">{t[language].noResultsDesc}</p>
                    </div>
                )}
            </div>
        </div>
