    ...
    <div className="container py-12 md:py-16">
        <div className="max-w-4xl mx-auto">
            <header className="mb-8">
                ...
                <Badge variant="secondary" className="mb-2">{getCategoryTranslation(post.category)}</Badge>
            ...
            {relatedPosts.length > 0 && (
                <section>
                    <h2 className="text-2xl font-bold tracking-tight mb-6">{t[language].relatedPosts}</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        {relatedPosts.map(relatedPost => {
                            const relatedImage = PlaceHolderImages.find(img => img.id === relatedPost.imageId);
                            return (
                                <Link key={relatedPost.id} href={`/blog/${relatedPost.slug}`} className="group">
                                    {relatedImage && (
                                        ...
                                        <Image
                                            ...
                                            data-ai-hint={relatedImage.imageHint || 'related post'}
                                        />
                                        </div>
                                    )}
                                    <h3 className="text-xl font-semibold group-hover:text-primary transition-colors">{relatedPost.title}</h3>
