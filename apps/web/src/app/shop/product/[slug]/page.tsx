...
import {
  aiSizeGuideRecommendation,
  AiSizeGuideRecommendationOutput,
} from '@/ai/flows/ai-size-guide-recommendation';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const sizeGuideFormSchema = z.object({
  chestMeasurementCm: z.coerce
    .number()
    .min(50, 'Please enter a valid measurement.')
    .max(200, 'Please enter a valid measurement.'),
  waistMeasurementCm: z.coerce
    .number()
    .min(50, 'Please enter a valid measurement.')
    .max(200, 'Please enter a valid measurement.'),
  hipMeasurementCm: z.coerce
    .number()
    .min(50, 'Please enter a valid measurement.')
    .max(200, 'Please enter a valid measurement.'),
...
export default function ProductDetailPage() {
  ...
  const [sizeRecommendation, setSizeRecommendation] =
    useState<AiSizeGuideRecommendationOutput | null>(null);
  ...
  const t = {
    en: {
      ...
      sizeGuideDesc:
        'Enter your measurements in CM to get a personalized size recommendation.',
      chest: 'Chest (cm)',
      waist: 'Waist (cm)',
      hips: 'Hips (cm)',
      inseam: 'Inseam (cm)',
  ...
  const productImages = product.images
    .map((imgId) => PlaceHolderImages.find((img) => img.id === imgId))
    .filter(Boolean);
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground">
              ({product.reviewCount} {t[language].reviews})
            </span>
          </div>
          ...
          <Dialog open={isSizeGuideOpen} onOpenChange={handleOpenChange}>
            ...
            <DialogContent className="sm:max-w-md">
              ...
              {!sizeRecommendation ? (
                <Form {...form}>
                  <form
                    onSubmit={form.handleSubmit(handleSizeGuideSubmit)}
                    className="space-y-4"
                  >
                ...
                <div className="space-y-4">
                  <Alert>
                    <AlertTitle className="font-bold text-lg">
                      {t[language].recommendationTitle}{' '}
                  ...
                  <DialogFooter>
                    <Button onClick={() => setSizeRecommendation(null)}>
                      {t[language].tryAgain}
                    </Button>
                  </DialogFooter>
                </div>
