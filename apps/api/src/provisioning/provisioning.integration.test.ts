...
describe('Provisioning Flow Integration', () => {
    ...
    describe('POST /api/provision', () => {
        ...
        it.each([
            { payload: { subdomain: '', superAdminKey: SUPER_ADMIN_KEY }, expected: 400, reason: 'Empty subdomain' },
            { payload: { subdomain: 'invalid_subdomain!', superAdminKey: SUPER_ADMIN_KEY }, expected: 400, reason: 'Invalid characters' },
            { payload: { subdomain: testSubdomain, adminEmail: 'admin@test.com', storeName: 'Store', plan: 'free', superAdminKey: 'short' }, expected: 400, reason: 'Short Admin Key' },
        ])('should handle error path: $reason (S5)', async ({ payload, expected }) => {
            const response = await request(app.getHttpServer())
                .post('/api/provision')
                .send(payload);
