...
describe('CLI Provisioning', () => {
    ...
    describe('Argument Parsing & Main Flow', () => {
        it('should execute successfully with valid arguments', async () => {
            ...
            expect(mockProvisioningService.provision).toHaveBeenCalledWith(expect.objectContaining({
                subdomain: 'test',
                plan: 'pro',
        ...
        it('should throw error for missing arguments', async () => {
            await expect(main(['--subdomain=only'])).rejects.toThrow('Missing required arguments');
        });

        it('should handle provisioning service failures', async () => {
            mockProvisioningService.provision.mockRejectedValueOnce(new Error('Provisioning Crash'));
            const args = ['--subdomain=x', '--email=x@x.com', '--password=p', '--store-name=X', '--quiet'];
