...
export class CloudflareService {
  ...
  async blockIp(
    ip: string,
    reason = 'S15 Honeytoken Violation'
  ): Promise<boolean> {
    if (!this.zoneId || !this.apiToken) {
      this.logger.warn(
        'S15: Cloudflare credentials missing. Skipping automated edge ban.'
      );
      return false;
    }
    ...
    try {
      ...
      if (result.success) {
        this.logger.log(
          `🛡️ S15: Successfully created Cloudflare edge-block for IP: ${ip}`
        );
        return true;
      }
      ...
      if (isAlreadyBlocked) {
        this.logger.debug(
          `S15: IP ${ip} is already blocked at Cloudflare edge.`
        );
        return true;
      }

      this.logger.error(
        `S15: Cloudflare API error: ${JSON.stringify(result.errors)}`
      );
      return false;
    } catch (error) {
      this.logger.error(
        `S15: Failed to communicate with Cloudflare API: ${(error as Error).message}`
      );
      return false;
    }
  }
