...
import { CreateBlueprintDto } from '../blueprints/dto/blueprint.dto.js';
...
async function verify() {
    ...
    // @ts-ignore - Inject mockDb into private property if constructor used drizzle(pool)
    ...
    try {
        ...
        if (mockAudit.log.mock.calls.length > 0) {
            ...
            if (logCall.action === 'BLUEPRINT_CREATED' && logCall.entityType === 'onboarding_blueprints') {
    ...
    mockDb.returning.mockResolvedValueOnce([{ id: 'bp-123', description: 'Updated' }]);

    try {
        await service.update(userId, 'bp-123', { description: 'Updated' });

        // Check latest audit log
        const latestLog = mockAudit.log.mock.calls[mockAudit.log.mock.calls.length - 1][0];
