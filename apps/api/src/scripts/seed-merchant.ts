...
import {
  banners
} from '@apex/db/schema/storefront/home';

import {
  categories
} from '@apex/db/schema/storefront/categories';

const TENANT_SUBDOMAIN = 'adel';
const SCHEMA_NAME = `tenant_${TENANT_SUBDOMAIN}`;

async function main() {
  console.log(`🚀 Starting Real Seed for: ${TENANT_SUBDOMAIN} (${SCHEMA_NAME})`);
  ...
  try {
    ...
    const categoriesData = [
      { slug: 'smartphones', name: 'Smartphones', description: 'Latest mobile phones' },
      { slug: 'laptops', name: 'Laptops', description: 'Powerful computing machines' },
      { slug: 'audio', name: 'Audio', description: 'Premium sound equipment' },
      { slug: 'watches', name: 'Watches', description: 'Smart and classic watches' }
    ];
    ...
    const [catSmartphones] = await db.select().from(categories).where(eq(categories.slug, 'smartphones')).limit(1);
    const [catLaptops] = await db.select().from(categories).where(eq(categories.slug, 'laptops')).limit(1);
    const [catAudio] = await db.select().from(categories).where(eq(categories.slug, 'audio')).limit(1);
    const [catWatches] = await db.select().from(categories).where(eq(categories.slug, 'watches')).limit(1);
    ...
    await db.insert(banners).values([
      {
        ...
        imageUrl: 'https://images.unsplash.com/photo-1519389950473-b7bc5658ab39?q=80&w=1600&h=600&auto=format&fit=crop',
        linkUrl: '/shop',
        ctaText: 'Shop Now',
        position: 'hero',
        priority: 10,
      ...
      },
      {
        ...
        imageUrl: 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?q=80&w=1600&h=600&auto=format&fit=crop',
        linkUrl: '/shop/audio',
    ...
    const productList = [
      ...
      },
      {
        slug: 'macbook-air-m3',
        name: 'Macbook Air M3',
        description: 'Leaning. Mean. M3. The world’s most popular laptop is even better.',
        price: '62000.00',
      ...
      },
      {
        ...
        description: 'Smarter. Brighter. Mightier. The most advanced health sensors.',
        price: '22000.00',
