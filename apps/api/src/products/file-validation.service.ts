...
const SIGNATURES: { prefix: Buffer; offset?: number; mime: string; ext: string }[] = [
    { prefix: Buffer.from([0xFF, 0xD8, 0xFF]), mime: 'image/jpeg', ext: 'jpg' },
    { prefix: Buffer.from([0x89, 0x50, 0x4E, 0x47]), mime: 'image/png', ext: 'png' },
...
export class FileValidationService {
     ...
     */
    validateAndSanitize(buffer: Buffer, originalFilename?: string): ValidatedFile {
        ...
        for (const sig of SIGNATURES) {
            ...
            if (buffer.subarray(offset, offset + sig.prefix.length).equals(sig.prefix)) {
                return {
