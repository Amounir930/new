...
try {
    EnvSchema.parse({
        ...
        ENCRYPTION_MASTER_KEY: 'valid_length_key_that_contains_word_test_12345_very_long',
...
try {
    EnvSchema.parse({
        JWT_SECRET: 'valid_length_key_that_contains_word_test_12345',
        ENCRYPTION_MASTER_KEY: 'valid_length_key_that_contains_word_test_12345_very_long', // Contains 'test'
        DATABASE_URL: 'postgresql://localhost:5432/db',
        REDIS_URL: 'redis://localhost:6379',
        MINIO_ENDPOINT: 'localhost',
        MINIO_ACCESS_KEY: 'admin',
    ...
    console.error('  ❌ Strict Production Mode: FAILED (Should have thrown error)');
} catch (e: any) {
    if (e.issues && e.issues.some((i: any) => i.message.includes('S1 Violation'))) {
        console.log('  ✅ Strict Production Mode: PASSED (Caught expected S1 Violation)');
    } else {
...
try {
    ...
    if (e.message.includes('S1 Violation') && e.message.includes('32 characters')) {
