...
describe('S3ValidationChecker', () => {
    ...
    describe('validate', () => {
        it('should pass clean code', async () => {
            ...
            vi.mocked(readFile).mockResolvedValue('const schema = z.object({}); const res = schema.safeParse(data);' as never);
        ...
        it('should detect useForm without zodResolver (FATAL)', async () => {
            ...
            vi.mocked(readFile).mockResolvedValue('const form = useForm({ defaultValues: {} });' as never);
        ...
        it('should detect API routes without validation (FATAL)', async () => {
            ...
            vi.mocked(readFile).mockResolvedValue('const body = await request.json(); console.log(body);' as never);
        ...
        it('should warn about inline email validation (WARNING)', async () => {
            vi.mocked(fg).mockResolvedValue(['file1.ts'] as never);
            vi.mocked(readFile).mockResolvedValue('if (email.includes("@")) { ... }' as never);
        ...
        it('should detect direct user input in template literals (FATAL)', async () => {
            vi.mocked(fg).mockResolvedValue(['file1.ts'] as never);
            vi.mocked(readFile).mockResolvedValue('const sql = `SELECT * FROM users WHERE id = ${request.body.id}`;' as never);
