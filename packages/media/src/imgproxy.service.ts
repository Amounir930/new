...
export class ImgproxyService {
    ...
    constructor(key: string, salt: string, sourceUrl: string = '') {
        ...
        this.sourceUrl = sourceUrl.endsWith('/') ? sourceUrl.slice(0, -1) : sourceUrl;
    }
     ...
     */
    public generateUrl(originalUrl: string, options: ImgproxyOptions = {}): string {
        ...
        if (this.sourceUrl && !originalUrl.startsWith('http')) {
            ...
            fullUrl = originalUrl.startsWith('/') ? `${this.sourceUrl}${originalUrl}` : `${this.sourceUrl}/${originalUrl}`;
        }
