...
export class V3Resolver {
     ...
     */
    resolveBlueprint(blueprint: Blueprint, context: ResolverContext): ResolvedBrick {
    ...
    resolveBrick(brick: Brick, context: ResolverContext): any {
        // 1. Check Condition
        if (brick.data?.condition && !this.evaluateCondition(brick.data.condition, context)) {
            return null;
        }
   ...
   */
    private resolveProps(props: Record<string, any>, context: ResolverContext): Record<string, any> {
     ...
     */
    private evaluateCondition(condition: string, context: ResolverContext): boolean {
        try {
