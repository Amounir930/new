...
export const BuilderCanvas: React.FC<BuilderCanvasProps> = ({ className }) => {
  const { currentTemplate, previewDevice, locale, selectSlot, selectedSlotId } =
    useBuilderStore();
  ...
  return (
    <div
      className={`flex-1 bg-muted/30 overflow-auto p-8 flex justify-center ${className || ''}`}
    >
      <div
        className={`bg-background shadow-2xl transition-all duration-300 min-h-full h-fit relative ${deviceWidths[previewDevice]}`}
