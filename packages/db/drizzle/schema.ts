...
export const int4range = customType<{ data: string }>({ dataType() { return "int4range"; } });
export const tstzrange = customType<{ data: string }>({ dataType() { return "tstzrange"; } });
...
export const auditLogs = pgTable(
  ...
  (table) => [
    ...
    index("idx_audit_timestamp").using(
      "btree",
      table.createdAt.asc().nullsLast().op("timestamptz_ops"),
    ),
  ],
