...
import { featureGates, subscriptionPlans, tenantQuotas, } from '../schema/governance';
export class GovernanceService {
     ...
     */
    async checkQuota(tenantId, resourceType, currentCount) {
    ...
    async isFeatureEnabled(tenantId, featureKey) {
        ...
        const gates = await db
            .select()
            .from(featureGates)
            .where(and(eq(featureGates.featureKey, featureKey), or(eq(featureGates.tenantId, tenantId), eq(featureGates.planCode, tenant.plan))));
        ...
        if (tenantGate)
            return !!tenantGate.isEnabled;
        const planGate = gates.find((g) => g.planCode === tenant.plan);
        if (planGate)
            return !!planGate.isEnabled;
