...
export class MasterControlService {
    ...
    async isMaintenanceMode(): Promise<boolean> {
        const config = await this.getConfig<{ enabled: boolean }>('master_maintenance', { enabled: false });
    ...
    async toggleMaintenance(enabled: boolean, message?: string): Promise<void> {
        await this.setConfig('master_maintenance', {
            enabled,
            message: message || 'Platform is currently undergoing maintenance. Please try again later.',
