 ...
 */
import { MasterBlueprintSchema } from '@apex/validators';
import { eq } from 'drizzle-orm';
import { db } from '../connection';
import { categories, shippingZones, storeLocations, tenantConfig, } from '../schema/storefront';
...
export class MasterBlueprintService {
     ...
     * Get the complete Master Blueprint for a tenant
     */
    async getMasterBlueprint(tenantId) {
        ...
        const getConfig = (key, defaultVal) => configRows.find((r) => r.key === key)?.value || defaultVal;
        // Get Shipping Zones
     ...
     */
    async updatePDP(_tenantId, pdpData) {
        await db
            .insert(tenantConfig)
            .values({
            key: 'pdp_config',
     ...
     */
    async updateCategoryBlueprint(categoryId, data) {
        await db
            .update(categories)
            .set({
            ...
            updatedAt: new Date(), // Assuming it exists, if not Drizzle handles it if defined
        })
            .where(eq(categories.id, categoryId));
    }
}
export const masterBlueprintService = new MasterBlueprintService();
//# sourceMappingURL=master-blueprint.service.js.map