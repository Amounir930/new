...
import { subscriptionPlans, tenantQuotas, featureGates } from '../schema/governance';
...
export class GovernanceService {
     ...
     */
    async checkQuota(tenantId: string, resourceType: 'products' | 'orders' | 'pages', currentCount: number): Promise<{ allowed: boolean; limit: number }> {
        const limits = await this.getTenantLimits(tenantId);

        let limit = 0;
        switch (resourceType) {
            case 'products': limit = limits.maxProducts; break;
            case 'orders': limit = limits.maxOrders; break;
            case 'pages': limit = limits.maxPages; break;
        }
