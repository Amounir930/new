...
mock.module('@apex/security', () => {
    return {
        EncryptionService: class {
            encrypt(val: any) { return { encrypted: 'ENC_' + val, iv: 'iv', tag: 'tag' }; }
            decrypt(val: any) { return 'DEC_' + val; }
        }
    };
});
...
describe('Arch-S8 Audit Logging Logic Verification', () => {
    ...
    it('should encrypt PII and persist to DB', async () => {
        ...
        const insertCallIndex = calls.findIndex(c => c && c.includes('INSERT INTO public.audit_logs'));
