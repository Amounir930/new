 ...
 */
export function decrypt(encryptedData, masterKey, fallbackKey) {
    const performDecryption = (keyStr) => {
    ...
    try {
        ...
        if (fallbackKey) {
            try {
                ...
                throw new Error('S7 Violation: Decryption failed with both primary and fallback keys.');
            }
        }
        throw error;
    }
...
export function hashSensitiveData(value) {
    const { createHmac } = require('node:crypto');
    const pepper = process.env.BLIND_INDEX_PEPPER || 'default-pepper-change-in-production';
...
let EncryptionService = class EncryptionService {
    ...
    validateMasterKey(key, isProduction) {
    ...
    decrypt(encryptedData, fallbackKey) {
