    ...
    describe('validate', () => {
        it('should validate a correct template structure', async () => {
            ...
            const mockConfig = {
                ...
                features: {
                    pages: {
                        home: true, productListing: true, productDetails: true, cart: true, checkout: true
                    }
                },
            ...
            vi.mocked(fs.readFileSync).mockImplementation((p: any) => {
                if (p.includes('template.config.json')) return JSON.stringify(mockConfig);
                if (p.includes('package.json')) return JSON.stringify({ dependencies: { '@apex/ui': '1.0.0' } });
                return '';
            });
        ...
        it('should fail if required files are missing', async () => {
            ...
            vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify({
                name: 'test', displayName: 'T', version: '1.0.0', description: 'D', category: 'general',
                author: { name: 'A', email: 'a@a.com' }, preview: { image: 'p.png' },
                features: { pages: { home: true, productListing: true, productDetails: true, cart: true, checkout: true } },
                requirements: { apexVersion: '1' }, locales: ['en'], rtlSupport: false
            }));
        ...
        it('should fail if template.config.json is invalid', async () => {
            vi.mocked(fs.existsSync).mockReturnValue(true);
            vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify({ name: 'INVALID NAME' })); // Not kebab-case
        ...
        it('should handle dependency errors (missing search page)', async () => {
            const mockConfig = {
                ...
                author: { name: 'A', email: 'a@a.com' }, preview: { image: 'p.png' },
                features: {
                    pages: { home: true, productListing: true, productDetails: true, cart: true, checkout: true, search: true }
                },
                requirements: { apexVersion: '1' }, locales: ['en'], rtlSupport: false
            };
            vi.mocked(fs.existsSync).mockImplementation((p: any) => {
