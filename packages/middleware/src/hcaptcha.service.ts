...
export class HCaptchaService {
    ...
    async verify(token: string, remoteIp?: string): Promise<boolean> {
        ...
        if (!this.secretKey) {
            this.logger.error('HCAPTCHA: S1 Violation - HCAPTCHA_SECRET_KEY not configured');
            return false;
        }
        ...
        try {
            ...
            this.logger.warn(`HCAPTCHA: Verification failed - Errors: ${JSON.stringify(data['error-codes'])}`);
            return false;
        } catch (error) {
