...
import { TenantIsolationMiddleware, TenantScopedGuard, SuperAdminOrTenantGuard, TenantRequest } from './tenant-isolation.middleware.js';
...
describe('TenantIsolationMiddleware', () => {
    ...
    describe('Subdomain Extraction', () => {
        it('should resolve tenant for localhost subdomains', async () => {
            mockDb.limit.mockResolvedValueOnce([{ id: 'uuid-1', subdomain: 'alpha', plan: 'pro', status: 'active' }]);
    ...
    describe('Tenant Validation', () => {
        ...
        it('should throw UnauthorizedException if tenant suspended', async () => {
            mockDb.limit.mockResolvedValueOnce([{ id: 'u1', subdomain: 'a', status: 'suspended' }]);
            await expect(middleware.use(req, res, next)).rejects.toThrow(UnauthorizedException);
        });
    });
});
...
describe('Guards', () => {
    ...
    describe('SuperAdminOrTenantGuard', () => {
        ...
        it('should block cross-tenant access for regular users', () => {
            ...
            expect(() => guard.canActivate(context)).toThrow('Cross-tenant access denied');
        });
