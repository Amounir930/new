    ...
    use(req: Request, res: Response, next: NextFunction): void {
        ...
        for (const botPattern of this.botUserAgents) {
            if (botPattern.test(userAgent)) {
                ...
                console.warn(`S11: Bot detected - User-Agent: "${userAgent}" | IP: ${req.ip}`);
        ...
        for (const pathPattern of suspiciousPaths) {
            if (pathPattern.test(req.url)) {
                console.warn(`S11: Suspicious path hit - Path: "${req.url}" | IP: ${req.ip}`);
                throw new ForbiddenException('S11 Violation: Security violation detected');
            }
        }
