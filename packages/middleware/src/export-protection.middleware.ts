...
export class ExportProtectionMiddleware implements NestMiddleware {
    async use(req: Request, res: Response, next: NextFunction) {
        const isExport = req.query.format === 'csv' || req.query.format === 'json' || req.path.includes('/export');
        ...
        if (isExport) {
            ...
            if (!user || (user.role !== 'admin' && user.role !== 'super_admin')) {
                throw new ForbiddenException('S1 Violation: Bulk export restricted to administrative accounts.');
            }

            // 2. Log Export Attempt to Audit Logs (Mandate #27)
            console.log(`[AUDIT] Export Attempt - User: ${user.email}, Resource: ${req.path}, Format: ${req.query.format}`);
