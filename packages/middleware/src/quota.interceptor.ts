 ...
 */
export const CheckQuota = (resource: 'products' | 'orders' | 'pages') => SetMetadata(QUOTA_KEY, resource);
...
export class QuotaInterceptor implements NestInterceptor {
    ...
    async intercept(context: ExecutionContext, next: CallHandler) {
        const resource = this.reflector.get<'products' | 'orders' | 'pages'>(QUOTA_KEY, context.getHandler());
