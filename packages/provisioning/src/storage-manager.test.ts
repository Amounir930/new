...
describe('StorageManager', () => {
    ...
    describe('createStorageBucket', () => {
        it.each([
            ...
            expect(mockMinioClient.makeBucket).toHaveBeenCalledWith(expectedBucket, expect.any(String));
            expect(mockMinioClient.setBucketTagging).toHaveBeenCalledWith(expectedBucket, expect.objectContaining({ plan }));
        });

        it('should throw if bucket already exists', async () => {
            mockMinioClient.bucketExists.mockResolvedValue(true);

            await expect(createStorageBucket('duplicate')).rejects.toThrow('Bucket already exists');
        });
    });
    ...
    describe('deleteStorageBucket', () => {
        ...
        it('should throw if bucket not empty and force is false', async () => {
            ...
            await expect(deleteStorageBucket('full-tenant')).rejects.toThrow('Bucket not empty');
        });
    ...
    describe('getStorageStats', () => {
        it('should calculate stats correctly', async () => {
            ...
            mockMinioClient.getBucketTagging.mockResolvedValue([{ Key: 'plan', Value: 'basic' }]);
            ...
            expect(stats.lastModified?.toISOString()).toBe(new Date('2024-01-02').toISOString());
        });
    });
});
