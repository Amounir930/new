...
export async function runTenantMigrations(
  ...
  try {
    ...
    console.log(`[Runner] Starting MANUAL migration for schema: ${schemaName}`);
    try {
      ...
      console.log(`[Runner] Migration completed for schema: ${schemaName}`);
    } catch (e: any) {
      if (e.code === '42P07') {
        // duplicate_table
        console.warn(
        ...
        console.error(`[Runner] Migration failed for ${schemaName}:`, e);
        throw e;
      }
    }
