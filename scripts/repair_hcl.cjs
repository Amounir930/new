...
hcl = hcl.replace(/column "embedding" \{\s+type\s+=\s+text/g, 'column "embedding" {\n    type = sql("vector(1536)")');
hcl = hcl.replace(/index "idx_product_embeddings" \{\s+columns\s+=\s+\[column\.embedding\]/g, 'index "idx_product_embeddings" {\n    columns = [column.embedding]\n    type    = "HNSW"');
...
const tablesToGuard = [
    ...
    { name: 'fulfillment_items', check: 'chk_fill_qty_pos', expr: 'quantity > 0' },
    { name: 'inventory_transfer_items', check: 'chk_transfer_qty_pos', expr: 'quantity > 0' },
    { name: 'checkout_sessions', check: 'chk_checkout_math', expr: 'COALESCE(total, 0) >= 0' }
];
...
tablesToGuard.forEach(t => {
    ...
    hcl = hcl.replace(tableRegex, (match) => {
        if (!match.includes(t.check)) {
            return match.replace(/\}$/, '  check "' + t.check + '" {\n    expr = "' + t.expr + '"\n  }\n}');
        }
        return match;
    });
});
...
hcl = hcl.replace(/target_url ~ '\^https:\/\/'/g, 'target_url ~ \'^https://(?!localhost|127\\.|10\\.|192\\.168\\.|172\\.(1[6-9]|2[0-9]|3[0-1]))\'');

// 8. GIN Index for Synonyms
hcl = hcl.replace(/table "search_synonyms" \{([^}]+)\}/g, (match, content) => {
    if (content.includes('column "synonyms"') && !content.includes('idx_synonyms_gin')) {
        return match.replace(/\}$/, '  index "idx_synonyms_gin" {\n    columns = [column.synonyms]\n    type    = "GIN"\n  }\n}');
    }
    return match;
});

// 9. Unique App Installation
hcl = hcl.replace(/table "app_installations" \{([^}]+)\}/g, (match, content) => {
    if (!content.includes('uq_tenant_app_name')) {
        return match.replace(/\}$/, '  unique "uq_tenant_app_name" {\n    columns = [column.tenant_id, column.app_name]\n  }\n}');
    }
    return match;
});
...
if (customerStart !== -1) {
    ...
    for (let i = customerStart; i < hcl.length; i++) {
        ...
        if (hcl[i] === '}') {
            braceCount--;
            if (braceCount === 0) { customerEnd = i + 1; break; }
        }
    }
    let table = hcl.substring(customerStart, customerEnd);
    table = table.replace(/column "tags" \{\s+type\s+=\s+text/g, 'column "tags" {\n    type = sql("text[]")');
...
hcl = hcl.replace(/table "product_views" \{([^}]+)\}/g, (match, content) => {
    if (!content.includes('idx_pv_session')) {
        return match.replace(/\}$/, '  index "idx_pv_session" {\n    columns = [column.session_id]\n  }\n}');
    }
    return match;
});

// 15. Customer Default Billing Fix
hcl = hcl.replace(/table "customer_addresses" \{([^}]+)\}/g, (match, content) => {
    if (!content.includes('uq_cust_default_billing')) {
        return match.replace(/\}$/, '  index "uq_cust_default_billing" {\n    unique = true\n    columns = [column.tenant_id, column.customer_id]\n    where = "is_default_billing = true"\n  }\n}');
    }
    return match;
});
