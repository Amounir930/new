...
console.log(color.gray(`   Scanning ${stagedFiles.length} staged file(s)...\n`));
...
for (const file of controllers) {
    ...
    if (hasBody) {
        ...
        // This is a heuristic.
        const hasValidationPipe = content.includes('ZodValidationPipe') || content.includes('ValidationPipe') || content.includes('ParseUUIDPipe') || content.includes('ParseIntPipe');
        ...
        if (!hasValidationPipe) {
            ...
            if (!hasDtoImport && !hasValidationPipe) {
                report('S3', 'Controller uses @Body() without visible ValidationPipe or DTO import.', file);
            }
        }
    }
}
